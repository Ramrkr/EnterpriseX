...
export function ViewOrderScreen() {
    ...
    const order = useSelector((state:RootState)=>state.order.viewingOrder);
    // const payment = order ? (order as any).paymentRecord as PaymentRecord : undefined;
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3" style={{ scrollbarWidth: "none" }}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-50 overflow-hidden">
          ...
          <div className="flex justify-between items-center px-4 py-3 border-t border-gray-100" style={{ background: "rgba(27,79,216,.04)" }}>
            ...
            <p className="text-base font-extrabold text-[#1B4FD8]">{fmt(totalAmount)}</p>
          </div>
        </div>
        <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-50">
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Payment Status</p>
          <div className="flex items-center justify-between">
            <PaymentBadge rec={order?.payment} total={totalAmount} />
            <div className="text-right">
              {order?.payment?.paymentMethod && <p className="text-xs font-bold text-gray-500 capitalize">{order?.payment.paymentMethod === "gpay" ? "GPay" : order?.payment.paymentMethod}</p>}
              {order?.payment?.status === "partial" && <p className="text-xs font-bold text-red-500">Balance: {fmt(totalAmount - (order?.payment?.amountPaid ?? 0))}</p>}
            </div>
          </div>
        </div>
      </div>
